...
from ..DSAnalysis import MokDS, DSChild
import os
...
def getdsPie(module_year_id):
    marks = marksList(module_year_id)
    dschild = DSChild(marks)
    return jsonify({"statusCode": 200,'data': dschild.getQCount()})
@ds_bp.route('/getbar/<int:module_year_id>' , methods = ["GET"])
def getdsbar(module_year_id):
    marks = marksList(module_year_id)
    dschild = DSChild(marks)
    return jsonify({"statusCode": 200,'data': dschild.getGradecounts()})
@ds_bp.route('/getline/<int:module_year_id>' , methods = ["GET"])
def getdsline(module_year_id):
    marks = marksList(module_year_id)
    dschild = DSChild(marks)
    return jsonify({"statusCode": 200,'data': dschild.getlines()})
@ds_bp.route('/getdist/<int:module_year_id>' , methods = ["GET"])
def getdsdist(module_year_id):
    marks = marksList(module_year_id)
    dschild = DSChild(marks)
    return jsonify({"statusCode": 200,'data': dschild.getDistributuions()})
@ds_bp.route('/getbox/<int:module_year_id>' , methods = ["GET"])
def getdsbox(module_year_id):
    marks = marksList(module_year_id)
    dschild = DSChild(marks)
    return jsonify({"statusCode": 200,'data': dschild.getBoxPlot()})
